...
author: "Group #6, Tea Hope - 20459579 "
...
1. **Victoria Gutierrez** - 20517120
    + [BIOL343_CC](https://github.com/Victoria-GA/BIOL343_TUT02-CC2_VictoriaGutierrez)
...
|---|---|---|
| population_type | Asexual or sexual population | Categorical (A/S) |
| ovary_width | Width of the flower ovary | mm |
| ovary_height | Height of the flower ovary | mm |
| flower_number | Number of flowers on a given plant | Integer |
...
```{r import-clean}
cc1_data_first <- read.csv("data/A1_Data_Excel.csv")
cc1_data_second <- cc1_data_first[1:46, 1:8] 
names(cc1_data_second)<- c(
    "population_type", 
    "population_name", 
    "latitude",
    "longitude",
    "ovary_width_mm",
    "ovary_height_mm", 
    "average_august_temperature",
    "flower_number"
)
cc1_data_second$ovary_area<-cc1_data_second$ovary_width_mm*cc1_data_second$ovary_height_mm
cc1_data_second$ovary_area

cc1_data<-cc1_data_second 
...
#why is none of this printing?????/ huge problems here but not sure what part is wrong? 
# Equation
...
$$\text{Ovary area} = \text{Ovary Height}\times \text{Ovary Width} $$

The units of ovary area are $mm^2$, with ovary height and ovary width as mm
...
area_means<-aggregate(ovary_area~population_type, 
                        data=cc1_data, 
                        FUN=mean)
...
plot1<-ggplot(cc1_data, aes(x= ovary_area)) + geom_histogram(binwidth=5) + 
facet_wrap(~ population_type) +
labs(title = "Distribution of Ovary Area by Population Type", x ="Ovary Area(mm^2)", y = "Frequency") + theme_classic()
...
![Solanum pimpinellifolium](images/Solanum_pimpinellifolium.jpg)
Source: [Wikispecies:free species directory] (https://species.wikimedia.org/wiki/Solanum_pimpinellifolium)
